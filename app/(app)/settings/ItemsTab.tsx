...
type FormState = { name: string; description: string; unitPrice: string; taxRate: string; unit: string; hsn: string }

const EMPTY_FORM: FormState = { name: "", description: "", unitPrice: "", taxRate: "", unit: "", hsn: "" }
...
export default function ItemsTab({ items: initial, orgId, currency }: { items: Item[]; orgId: string; currency: string }) {
  ...
  function openEdit(item: Item) { setForm(itemToForm(item)); setEditId(item.id); setShowForm(true) }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (editId) {
      const res = await fetch(`/api/settings/items/${editId}`, {
        method: "PATCH",
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-gray-50 rounded-xl p-4 space-y-3">
          <p className="text-sm font-medium text-gray-700">{editId ? "Edit item" : "New item"}</p>
